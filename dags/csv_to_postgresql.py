...
def add_csv_to_postgres(file_name, table_name, primary_key, **context):
...
def reload_csv_to_postgres(file_name, table_name, **context):
    csv_path = f"/opt/airflow/data/{file_name}"
    ...
    try:
        ...
        trunc_sql =f"""truncate table {table_name} cascade"""
...
with DAG(
    ...
    load_deal = PythonOperator(
        ...
        python_callable=add_csv_to_postgres,
    ...
    load_product = PythonOperator(
        ...
        python_callable=reload_csv_to_postgres,
        op_kwargs={"file_name": "product_info.csv", "table_name": "rd.product"}
    )
